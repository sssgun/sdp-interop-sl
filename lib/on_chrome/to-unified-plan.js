...
module.exports = function(desc, cache) {
    ...
    session.media.forEach(function (bLine) {
        ...
        ssrcGroups.forEach(function (ssrcGroup) {
            ssrcGroup.ssrcs.forEach(function(ssrc){ ssrcsInUse.push(ssrc); }); //update in use ssrcs so we don't accidentally override it
    ...
    if(desc.type === "answer") {
        ...
        if(remoteRef === undefined)
            throw Error("remote cache required to generate answer - why didn't you munge it into plan b like a good boy?");
